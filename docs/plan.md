...
- [x] `GET /api/seed/test-package?auth=<SESSION_SECRET>` — endpoint import dari `data.json`, dilindungi query param `auth` (bukan session, karena dipanggil manual/tool eksternal) — sudah dikecualikan dari guard `proxy.ts` lewat `PUBLIC_PREFIXES = ["/api/seed/"]` yang sudah ada dari Fase 5.1
- [x] Idempotent per `TestPackage.name` — paket yang sudah ada di-skip, tidak dobel, aman dipanggil berkali-kali
- [x] Transaksi per soal (bukan per paket seperti demo seed) — satu soal gagal (mis. `questionContextRef` tidak ketemu) dicatat sebagai error di response & log, proses lanjut ke soal berikutnya, tidak rollback seluruh paket. Sesuai `database.md`: pelanggaran constraint = sinyal error ekstraksi, dilaporkan (bukan di-skip diam-diam)
- [x] `src/app/api/seed/test-package/types.ts` — kontrak TypeScript (`SeedData`/`SeedTestPackage`/`SeedTestPackageItem`/`SeedQuestion`/`SeedQuestionChoice`/`SeedQuestionContext`), `data.json` di-import langsung sebagai modul (`resolveJsonModule` sudah aktif di `tsconfig.json`)
- [x] `docs/seed.md` — dokumentasi lengkap kontrak JSON untuk konteks tool/AI scraping eksternal: field-by-field, daftar nilai enum (`jlptLevel`/`section`/`mondaiType`), **aturan penomoran `session` per level** (N1/N2 = 2 sesi, N3-N5 = 3 sesi — beda pembagian section per sesi), rekap markup teks, validasi umum, contoh JSON lengkap
- [x] Logging aktif (`[seed:test-package] CREATE/SKIP/ERROR/DONE ...`) di setiap langkah — bisa ditrack dari log server, plus ringkasan terstruktur di response JSON (`packagesSeeded`/`packagesSkipped`/`questionsSeeded`/`errors`)
- [x] Verifikasi: dites dengan payload sementara (1 paket, 2 mondai, 1 context, 1 soal dengan `questionContextRef` sengaja rusak) — run pertama seed 2/3 soal + 1 error tercatat rapi, run kedua skip (idempoten), data test dibersihkan lagi dari DB setelah verifikasi. `npm run build` & `npm run lint` bersih
