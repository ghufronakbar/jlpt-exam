...
- All 89 answers match the printed answer table.
...
- `npm run build`, fixture validation, deterministic count/answer checks, media-reference checks, JSON parsing, and trailing-whitespace checks pass.
...
- The あらきや flyer is represented as Markdown tables in `storyText`, retaining prices, periods, weekdays, and contact details.
...
- The fourth distractor for 表記 question 12 is visually indistinguishable from `飲みました` in the available scan; it is transcribed as printed rather than replaced with an invented character.
